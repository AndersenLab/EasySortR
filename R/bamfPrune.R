...
#'
#' Prune based on binned IQR multiples eminating from the center of the distribution. Anomalies are not removed if they account for more than 5\% of the population. 
#' 
...
bamfPrune <- function(data) {
    ...
    # Make sure that the data being fed into the pruning fuction is in long format
    ...
    dataWithOutliers <- data %>%
        ...
        # Group on condition and trait, the, calculate the first and third quartiles for each of the traits
        ...
        # Group on condition and trait, then sum the total number of data points in each of the IQR multiple bins
        dplyr::group_by(condition, trait)%>%
        ...
        # Group on condition and trait, then check to see if the number of points in each bin is more than 5% of the total number of data points
        dplyr::group_by(condition, trait)%>%
        ...
        # Group on condition and trait, then filter out NAs in any of the added columns
        dplyr::group_by(condition, trait)%>%
    ...
    # Select the necessary columns rename the outlier calling columns, and arrange by condition, row, and columns
