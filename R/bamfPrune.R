...
bamfPrune <- function(data) {
    dataWithOutliers <- data %>%
        ...
        dplyr::mutate(cut1h = q3+(iqr*2),
               ...
               cut6l = q1-(iqr*10),
               cut6h = q3+(iqr*10))%>%
        dplyr::left_join(plate, ., by=c("condition", "variable"))%>%
        dplyr::mutate(onehs = ifelse( cut2h > value & value >= cut1h,1,0),
