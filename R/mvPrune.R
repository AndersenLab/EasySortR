...
mvPrune <- function(data, mvCols = c("f.L1", "f.ad", "mean.TOF", "mean.EXT")){
    ...
    outlierFrameWithOutliers <- outlierFrame %>% mutate(mvOutlier=!as.logical(mvoutlier::pcout(outlierFrame, explvar = 0.99, crit.M1 = 0.35, crit.c1=2.5, critM2=0.5, crit.c2= 0.995, outbound = 0.1, cs = 0.45, makeplot = FALSE)$wfinal01))
